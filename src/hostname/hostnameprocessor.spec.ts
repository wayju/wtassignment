...
describe('Hostname processor', () => {
  ...
  it('Results should be returned', async () => {
    ...
    expect(results.find((r) => r.hostname === 'google.com')?.hostname).toBe(
      'google.com'
    );
    expect(results.find((r) => r.hostname === 'google.com')?.ip).toBe(
      '2.2.2.2'
    );
    expect(results.find((r) => r.hostname === 'sub.google.com')?.hostname).toBe(
      'sub.google.com'
    );
    expect(results.find((r) => r.hostname === 'sub.google.com')?.ip).toBe(
      '3.3.3.3'
    );
    expect(results.find((r) => r.hostname === 'facebook.com')?.hostname).toBe(
      'facebook.com'
    );
    expect(results.find((r) => r.hostname === 'facebook.com')?.ip).toBe(
      '4.4.4.4'
    );
  });
  ...
  it('Multiple IPs should be returned for multiple A records', async () => {
    ...
    expect(results.find((r) => r.hostname === 'microsoft.com')?.hostname).toBe(
      'microsoft.com'
    );
    expect(results.find((r) => r.hostname === 'microsoft.com')?.ip).toBe(
      '20.70.246.20 20.112.250.133 20.131.239.246 20.76.201.171'
    );
  });
});
